...
- Strict pre-flight validation of GPT Image 2 constraints (size, aspect, pixel count, transparent background format, 2.5-only quality levels, `output_compression`, `stream` + `n`, and unsupported `response_format` usage on OpenAI GPT image models).
...
Derived from [GargantuaX/openskills](https://github.com/GargantuaX/openskills) `gpt-image-2`, extended for GPT Image 2.5 and OpenAI's image prompting guide. Tracked in dotfiles at `~/.claude/skills/gpt-image`.
...
├─ references/
│  ├─ prompting.md          # Model choice and prompt patterns
...
```bash
python3 -m unittest discover -s ~/.claude/skills/gpt-image/tests -p "test_*.py"
```
